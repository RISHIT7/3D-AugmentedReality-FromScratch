...
class TagTracker:
    ...
    def update(self, tag_corners):
        if not self.is_initialized:
            for i in range(4):
                self.filters[i].state[:2] = tag_corners[i]
        ...
        for i in range(4):
            ...
            updated = self.filters[i].update(tag_corners[i][0], tag_corners[i][1])
            updated_corners.append(updated)
        
        return np.array(updated_corners, dtype=np.float32)
...
def get_projection_matrix(camera_matrix, homography):
    ...
    pose = np.column_stack((r1, r2, r3, t))
    return pose
...
def process_frame(frame, template_img=None, camera_matrix=None):
    ...
    for tag in stable_tags:
        ...
        if tid not in tag_trackers:
            tag_trackers[tid] = TagTracker()
        
        smoothed_corners = tag_trackers[tid].update(rect)
        tag['corners'] = smoothed_corners
        
        if template_img is not None:
            h_temp, w_temp = template_img.shape[:2]
            src_pts = np.array([[0, 0], [w_temp, 0], [w_temp, h_temp], [0, h_temp]], dtype="float32")
            H_template_to_screen = CustomCV2.getPerspectiveTransform(src_pts, smoothed_corners)
            H_inv = np.linalg.inv(H_template_to_screen)
            if CPP_AVAILABLE:
                frame = custom_cv2_cpp.overlay_image_cpp(frame, template_img, H_inv)
                
        tag_half_size = 1.0
        obj_pts = np.array([[-tag_half_size, -tag_half_size],
                            [tag_half_size, -tag_half_size],
                            [tag_half_size, tag_half_size],
                            [-tag_half_size, tag_half_size]], dtype=np.float32)
        H_3d = CustomCV2.getPerspectiveTransform(src_pts, smoothed_corners)
        pose_mat = get_projection_matrix(camera_matrix, H_3d)
        angle = math.atan2(pose_mat[1, 0], pose_mat[0,0])
        angle_deg = math.degrees(angle)
        tag['rotation'] = angle_deg
        
        rect = smoothed_corners
